...
const FormBuilder: React.FC<FormBuilderProps> = ({
  ...
  const handleMultiSelectChange = (
    ...
    if (currentValue.includes(selectedItem)) {
      updatedItems = currentValue.filter((item: string) => item !== selectedItem);
    } else {
  ...
  return (
    <div className="w-full" ref={errorRef}>
      {(formErrors.length > 0 || customErrors.length > 0) && (
        <div className="bg-red-100 rounded-md p-4 my-4">
      ...
      {rows.map((row, rowIndex) => (
        <div key={`row-${rowIndex}`} className="flex flex-wrap gap-6 mb-4">
          {row.map((field, index) => {
              ...
              return (
                <div key={`${rowIndex}-${index}`} className="flex-1">
                  {React.isValidElement(field.component)
                    ? field.component
                    : field.component
                    ? React.createElement(field.component as React.ComponentType<any>, {
      ...
      {(functions?.submit || functions?.cancel) && (
        <div
          className={`flex ${
            functions.modal ? "justify-between" : "justify-end"
          } gap-3 mt-4`} 
        >
          {functions?.cancel && (
            <button
              onClick={functions.cancel.function}
              className="border border-gray-400 bg-white text-gray-800 px-4 py-2 rounded-md font-semibold text-sm hover:bg-gray-100"
            >
