    ...
    <div className="space-y-3">
      ...
      <div className="space-y-3 border rounded-xl p-4 bg-white">
                ...
                <button
                  onClick={() => {
                    ...
                    if (itemToRemove) {
                      onChange(
                        itemToRemove.value,
                        selectedItems.filter((i) => i !== itemToRemove.value)
                      );
                    }
                  }}
        ...
        <div className="flex items-center bg-gray-50 rounded-md px-3 py-2">
          ...
          <input
            type="text"
            className="flex-1 ml-2 text-base text-gray-700 outline-none bg-transparent"
        ...
        <button
          className="flex items-center py-2 border-b border-gray-200 hover:bg-gray-50 w-full"
        ...
        <div className="max-h-40 overflow-y-auto space-y-2">
          {filteredItems.length > 0 ? (
            filteredItems.map((item) => (
              <button
                key={item.value}
                className="flex items-center w-full py-2 hover:bg-gray-50"
            ...
            <div className="py-3 text-center text-gray-500">No matches found</div>
          )}
        </div>
      </div>
    </div>
